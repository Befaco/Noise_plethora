...
class Rwalk_LBit : public Plugin {
  ...
  public:
    ...
    Rwalk_LBit(const Rwalk_LBit&) = delete;
    Rwalk_LBit& operator=(const Rwalk_LBit&) = delete;
...
REGISTER_PLUGIN(Rwalk_LBit); // this is important, so that we can include the plugin in a bank
